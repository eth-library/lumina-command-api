...
| `502` from `/pipeline/*` only, **every time** | Cloud Run cannot reach `lumina-box01.ethz.ch:4200`, or Prefect is down there | Check the Engine host first; if the host is up but unreachable from Cloud Run, the network path needs a decision of its own ([ADR 0007](../adr/0007-prefect-read-only-proxy.md)) |
| `502` from `/pipeline/*` **intermittently** — a few requests succeed after quiet, then refusals within ~100 ms | A per-source connection limit on the path (ETH perimeter or Cloud NAT ports) is being hit by a burst of new connections | Read the errno in the log line `Prefect API unreachable: …` — `Errno 111` is a firewall, `Errno 99` is NAT ports. The code side is [ADR 0009](../adr/0009-shared-keepalive-client-for-prefect.md); the infrastructure side is Cloud NAT dynamic port allocation and, for 111, a question to ETH network operations |
